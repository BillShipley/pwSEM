...
*remove.edges*: a character object specifying which pairs of variables a known to not have an edge in the partially oriented dependency graph. For example, remove.edges="X2-X1" means that you know that there cannot be an edge X2o--oX1 in this graph (including arrowheads pointing into either or both of X2 and X1).  In other words, you know that X2 cannot be a causal parent of X1, X1 cannot be a causal parent of X2 and, finally, that X1 and X2 cannot be causal siblings of an unmeasured common cause of both. For more than one forbidden edge, insert a RETURN after each pair.
...
If you wanted to forbid edges between XF and XM and between XP and XM, you would do this:
...
CI.algorithm(dat=nested_data[,-3],family=data.frame(XR="binomial",
            XP="poisson"),nesting=nesting.structure,alpha.reject=0.3,
            edges.to.remove=no.edges,smooth=T)
```
